#include <iostream>
using namespace std;
int main(){
    
    bool equal_result {false};
    bool not_equal_result{false};

    //int num1{}, num2{};

    /*
    cout<< boolalpha ;  //will display true/false instead of 1-0 for booleans 
    cout <<" Enter two integers separated by a space: ";
    cin >> num1 >> num2 ;
    equal_result = (num1==num2);
    not_equal_result=( num1 != num2);
    cout<<" Comparison result(equals): "<<equal_result<<endl;
    cout<<" Compariosn result(not equal): "<< not_equal_result<<endl;
    */  

   /*
   char char1(), char2();
   cout<<" enter two characters separated by space "<<endl;
   cin >> char1 >> char2 ;
   equal_result = (char1==char2);
   not_equal_result =(char1!=char2);
   cout<<" Comparison rsult(equal) :"<< equal_result << endl;
   cout<<" Comparison result(not equals) :"<< not_equal_result <<endl;
    */

   cout<< boolalpha;

   double double1, double2 ;
   cout<<" Enter two doubles separated by space:";
   cin>> double1 >> double2 ;
   equal_result=( double1==double2);
   not_equal_result=( double1 != double2);
   cout<< "Comparison result(for equals) "<< equal_result <<endl;
   cout<< " Comparison result (for not equals)"<< not_equal_result ;
   


    return 0 ;

}